    ...
    const mapCatalog = catalog.filter(item=>!item.map_excluded);
    const mapBenchmarkCount = data.summary.map_benchmark_group_count || mapCatalog.length;
    ...
    const benchmarkTypeById = new Map((data.taxonomy?.benchmark_types||[]).map(item=>[item.id,item]));
    const secondaryTagById = new Map((data.taxonomy?.secondary_tags||[]).map(item=>[item.id,item]));
    ...
    const secondaryTagLabels=item=>(item.secondary_tags||[]).map(id=>{const rule=secondaryTagById.get(id);return rule?t(rule.label_en,rule.label_zh):id;});
    const benchmarkTypeLabel=item=>{const rule=benchmarkTypeById.get(item.benchmark_type);return rule?t(rule.label_en,rule.label_zh):(item.benchmark_type||t('Atomic benchmark','单项 Benchmark'));};
    const itemSearchText = item => `${displayName(item)} ${item.benchmark_name} ${item.domain} ${item.metric_name} ${benchmarkTypeLabel(item)} ${secondaryTagLabels(item).join(' ')} ${(item.search_models||[]).join(' ')}`.toLowerCase();
    ...
    function updateSemanticZoom(){
      ...
      const focusRule=domainViewRules.find(rule=>rule.id===state.domain);$('mapCoverage').textContent=focusRule?`${visibleNodes.length} / ${featured.length} ${t('benchmark families in this field','个该领域 Benchmark 家族')}`:`${visibleNodes.length} / ${formatCount(mapBenchmarkCount)} ${t('benchmarks shown','个 Benchmark 已显示')}`;
    ...
    function renderRegistry(){$('registryBody').innerHTML=filteredCatalog().slice(0,100).map(item=>{const macro=getMacro(item);const subfield=getViewSubfield(item);const score=modelScore(item);const model=selectedModel();const config=score?.n&&score.n!=='Standard'?` · ${score.n}`:'';const taxonomyDetail=[item.benchmark_type==='composite_index'?benchmarkTypeLabel(item):macro.label,subfield.label,...secondaryTagLabels(item)].filter(Boolean);return `<tr data-key="${esc(item.rank_group_key)}"><td><b>${esc(displayName(item))}</b><small>${esc(item.metric_name)} · ${esc(item.score_unit)}${item.benchmark_variant?` · ${esc(item.benchmark_variant)}`:''}</small></td><td>${esc(taxonomyDetail[0])}<small>${esc(taxonomyDetail.slice(1).join(' · '))}</small></td><td>${item.model_count}<small>base models</small></td><td>${item.vendor_count}</td><td class="score">${esc(score?formatScore(score.s,score.u):formatScore(item.best_score,item.score_unit))}<small>${esc(model?.model_name||item.best_model)}${esc(config)}</small></td><td>${protocolChips(item)}</td></tr>`}).join('');document.querySelectorAll('#registryBody tr').forEach(row=>row.addEventListener('click',()=>{const item=catalog.find(entry=>entry.rank_group_key===row.dataset.key);const score=modelScore(item);selectBenchmark(row.dataset.key,score?.g||'',state.modelId);}));}
    ...
    function openSearchResult(key){const item=catalog.find(entry=>entry.rank_group_key===key);if(!item)return;state.query='';$('search').value='';closeSearch();if(item.map_excluded){state.domain='all';switchMode('registry');selectBenchmark(key);syncUrl();return;}enterDomainFocus(getMacro(item).id,false);const representative=mapRepresentativeFor(item);switchMode('map');selectBenchmark(key);setTimeout(()=>centerOn((representative||item).rank_group_key),0);syncUrl();}
    ...
    $('railCount').textContent=`${data.summary.result_count} ROWS · ${data.summary.model_count} MODELS · ${mapBenchmarkCount} BENCHMARKS`;
    ...
    $('headerBenchmarks').textContent=formatCount(mapBenchmarkCount);
    ...
    $('mapCoverage').textContent=`${featured.length} / ${formatCount(mapBenchmarkCount)} ${t('landmarks shown','个 Benchmark 已显示')}`;
