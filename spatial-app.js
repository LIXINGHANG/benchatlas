    ...
    async function selectBenchmark(key,preferredProtocolId,preferredModelId){
      ...
      const related=highlightRelations(mapKey);const groupedRows=groupRowsByModel(page.rows);const comparisonGroups=comparisonGroupsForRows(page.rows);const macro=getMacro(item);const subfield=getSubfield(item);
    ...
    function filteredCatalog(){const q=state.query.toLowerCase();return catalog.filter(item=>(state.domain==='all'||getMacro(item).id===state.domain)&&(!state.modelId||Boolean(item.model_scores?.[state.modelId]))&&(!state.safetyOnly||isSafety(item))&&(!q||itemSearchText(item).includes(q)));}
    ...
    function renderRegistry(){$('registryBody').innerHTML=filteredCatalog().slice(0,100).map(item=>{const macro=getMacro(item);const score=modelScore(item);const model=selectedModel();const config=score?.n&&score.n!=='Standard'?` · ${score.n}`:'';return `<tr data-key="${esc(item.rank_group_key)}"><td><b>${esc(displayName(item))}</b><small>${esc(item.metric_name)} · ${esc(item.score_unit)}${item.benchmark_variant?` · ${esc(item.benchmark_variant)}`:''}</small></td><td>${esc(macro.label)}<small>${esc(getSubfield(item).label)}${isSafety(item)?' · safety':''}</small></td><td>${item.model_count}<small>base models</small></td><td>${item.vendor_count}</td><td class="score">${esc(score?formatScore(score.s,score.u):formatScore(item.best_score,item.score_unit))}<small>${esc(model?.model_name||item.best_model)}${esc(config)}</small></td><td>${protocolChips(item)}</td></tr>`}).join('');document.querySelectorAll('#registryBody tr').forEach(row=>row.addEventListener('click',()=>{const item=catalog.find(entry=>entry.rank_group_key===row.dataset.key);const score=modelScore(item);selectBenchmark(row.dataset.key,score?.g||'',state.modelId);}));}
    ...
    function openSearchResult(key){const item=catalog.find(entry=>entry.rank_group_key===key);if(!item)return;state.safetyOnly=false;$('safetyToggle').checked=false;state.query='';$('search').value='';closeSearch();enterDomainFocus(getMacro(item).id,false);const representative=mapRepresentativeFor(item);switchMode('map');selectBenchmark(key);setTimeout(()=>centerOn((representative||item).rank_group_key),0);syncUrl();}
