const data = window.BENCHATLAS_DATA;
    ...
    const state = {scale:.72,x:40,y:18,drag:false,startX:0,startY:0,baseX:0,baseY:0,domain:'all',safetyOnly:false,query:'',selected:null,mode:'map',activeVariants:[],activeItem:null,matrixModelIds:new Set()};
    ...
    function selectBenchmark(key){
      ...
      $('comparisonSelect').onchange=event=>renderComparison(event.target.value);renderComparison(comparisonGroups[0]?.id);$('inspector').classList.add('open');$('benchmarkLink').href=`/benchmarks/${slugify(item.rank_group_key)}/`;showToast(`${displayName(item)} · ${groupedRows.length} unique models`);
    }
    ...
    function renderMatrix(){
      ...
      $('matrixTopModels').onclick=()=>{state.matrixModelIds.clear();availableModels.slice(0,7).forEach(model=>state.matrixModelIds.add(model.id));renderMatrix();$('matrixModelMenu').classList.add('open');};
      $('matrixModelMenu').querySelectorAll('input').forEach(input=>input.addEventListener('change',event=>{if(event.target.checked&&state.matrixModelIds.size>=12){event.target.checked=false;showToast('Matrix supports up to 12 models');return;}if(event.target.checked)state.matrixModelIds.add(event.target.value);else state.matrixModelIds.delete(event.target.value);renderMatrix();$('matrixModelMenu').classList.add('open');}));
    }
