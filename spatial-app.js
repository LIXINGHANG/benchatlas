const data = window.BENCHATLAS_DATA;
    ...
    async function selectBenchmark(key,preferredProtocolId){
      ...
      $('inspectorTitle').textContent=displayName(item);$('inspectorSub').textContent='Loading benchmark details…';$('ranking').innerHTML='<p class="method">Loading reported scores…</p>';
      let page;
    ...
    const requestedBenchmark=initialHash.get('benchmark');const initialBenchmark=catalog.some(item=>item.rank_group_key===requestedBenchmark)?requestedBenchmark:featured[0].rank_group_key;
    if(!positions.has(initialBenchmark)&&state.mode==='map')state.mode='registry';
    if(window.matchMedia('(max-width:650px)').matches&&!initialHash.has('view'))state.mode='registry';
    switchMode(state.mode);selectBenchmark(initialBenchmark,state.initialProtocol);
