    ...
    async function renderOverallRanking(){
      ...
      try{const overall=await loadRankingData();const rankings=overall.rankings||[];$('rankingModelCount').textContent=rankings.length;$('rankingGroupCount').textContent=overall.benchmarkGroupCount||'—';
        $('overallRankingBody').className='';$('overallRankingBody').innerHTML=`<table class="overall-ranking"><thead><tr><th>Rank</th><th>Model</th><th>RPI</th><th>Coverage</th><th>Confidence</th></tr></thead><tbody>${rankings.map(row=>{const model=modelCatalog.find(item=>item.model_name===row.model_name);const path=model?`/models/${encodeURIComponent(model.model_id)}/`:'#';const width=Math.max(0,Math.min(100,Number(row.index_score)||0));return `<tr data-path="${esc(path)}"><td class="overall-rank">${String(row.overall_rank).padStart(2,'0')}</td><td class="model-cell"><b>${esc(row.model_name)}</b><small>${esc(row.vendor)}</small></td><td class="rpi-cell"><div class="rpi-value"><span>${esc(row.index_score)}</span><small>RPI</small></div><div class="rpi-bar"><i style="width:${width}%"></i></div></td><td class="coverage-cell">${row.benchmark_count} benchmarks<small>${row.domain_count} capability domains · raw ${row.raw_score}</small></td><td class="confidence-cell"><span class="confidence-pill">${esc(row.confidence)}</span><small>${row.report_count} source reports</small></td></tr>`;}).join('')}</tbody></table>`;
