    ...
    async function applyUrlState(){const params=new URLSearchParams(location.hash.replace(/^#/,''));const requestedMode=params.get('view');state.mode=['map','registry','matrix','ranking'].includes(requestedMode)?requestedMode:'map';const requestedDomain=params.get('domain');state.domain=macroRules.some(rule=>rule.id===requestedDomain)?requestedDomain:'all';state.safetyOnly=params.get('safety')==='1';state.matrixModelIds=new Set((params.get('models')||'').split(',').filter(Boolean));if($('domainSelect'))$('domainSelect').value=state.domain;if($('safetyToggle'))$('safetyToggle').checked=state.safetyOnly;const benchmark=params.get('benchmark');const hasBenchmark=catalog.some(item=>item.rank_group_key===benchmark);if(!hasBenchmark)state.selected=null;applyFilters();if(hasBenchmark&&!positions.has(benchmark)&&state.mode==='map')state.mode='registry';switchMode(state.mode);if(hasBenchmark&&state.mode!=='ranking')await selectBenchmark(benchmark,params.get('protocol')||'');else if(!hasBenchmark)closeInspector(false);}
