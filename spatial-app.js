    ...
    const macroRules = [
      {id:'reason',label:t('Reasoning & Knowledge','推理与知识'),color:'#27548a',center:[300,270],domains:['reasoning','math','general','general_capability']},
      {id:'code',label:t('Coding & Software Engineering','编程与软件工程'),color:'#0f766e',center:[900,270],domains:['coding']},
      {id:'agent',label:t('Agents & Computer Use','智能体与计算机操作'),color:'#a96812',center:[1520,270],domains:['agent','computer_use','business_simulation','healthcare_agent']},
      {id:'multi',label:t('Multimodal & Perception','多模态与感知'),color:'#67528d',center:[300,800],domains:['multimodal','vision','video','document']},
      {id:'language',label:t('Language & Long Context','语言与长上下文'),color:'#4f7d35',center:[900,800],domains:['language','multilingual','long_context']},
      {id:'expert',label:t('Expert & Frontier Domains','专业与前沿领域'),color:'#8b4b36',center:[1520,800],domains:['science','health','research','professional','expert_tasks','cybersecurity','security','self_improvement']}
    ];
    ...
    const subfieldRules = {
      ...
      language:[[t('Long context','长上下文'),/long|context|needle|mrcr|graphwalk|memory/i],[t('Multilingual','多语言'),/multi|language|mmlu-prox|global/i],[t('Translation','翻译'),/wmt|translation|xcomet/i],[t('Language understanding','语言理解'),/.*/]],
    ...
    const mapFamilyRules = [{id:'healthbench',pattern:/^healthbench(?:\s|$)/i,representative:/^healthbench$/i}];
    const mapFamilyRule = item => mapFamilyRules.find(rule=>rule.pattern.test(String(item.benchmark_name||'')));
    const mapFamilyKey = item => mapFamilyRule(item)?.id||item.benchmark_family_id||item.rank_group_key;
    const preferredMapRepresentative = item => mapFamilyRule(item)?.representative.test(String(item.benchmark_name||''))||false;
    const mapTierLimits={field:7,detail:9,deep:10};
    const selectFieldLandmarks = rule => {const families=new Map();for(const item of catalog){if(getMacro(item.domain).id!==rule.id)continue;const family=mapFamilyKey(item);const current=families.get(family);if(!current||preferredMapRepresentative(item)&&!preferredMapRepresentative(current))families.set(family,item);}return [...families.values()].slice(0,mapTierLimits.deep);};
    const featured = macroRules.flatMap(selectFieldLandmarks);
    const featuredByFamily = new Map(featured.map(item=>[mapFamilyKey(item),item]));
    const mapRepresentativeFor = item => item?featuredByFamily.get(mapFamilyKey(item))||null:null;
    const featuredPriority = new Map();
    const featuredLod = new Map();
    macroRules.forEach(rule=>featured.filter(item=>getMacro(item.domain).id===rule.id).forEach((item,index)=>{featuredPriority.set(item.rank_group_key,index<3?'core':'support');featuredLod.set(item.rank_group_key,index<mapTierLimits.field?'field':index<mapTierLimits.detail?'detail':'deep');}));
    const lodMinScale={field:.58,detail:.95,deep:1.15};
    const itemMinScale=item=>featuredPriority.get(item.rank_group_key)==='core'?.35:lodMinScale[featuredLod.get(item.rank_group_key)]||.58;
    ...
    const protocolCode=index=>`${t('Protocol','协议')} ${String.fromCharCode(65+index)}`;
    ...
    function positionNodes(){
      ...
      groups.forEach((items,id)=>{const subgroups=new Map();items.forEach(item=>{const subfield=getSubfield(item);if(!subgroups.has(subfield.id))subgroups.set(subfield.id,{...subfield,items:[]});subgroups.get(subfield.id).items.push(item);});fieldGroups.set(id,{items,entries:[...subgroups.values()].slice(0,4)});});
    ...
    function renderMap(){
      ...
      $('nodes').innerHTML=featured.map(item=>{const p=positions.get(item.rank_group_key);const risk=isSafety(item);const lod=featuredLod.get(item.rank_group_key);const nodeScale=lod==='field'?Math.min(1.08,.84+item.model_count/80):Math.min(.9,.72+item.model_count/100);return `<button class="node ${risk?'risk':''} ${lod!=='field'?'expanded-landmark':''}" data-key="${esc(item.rank_group_key)}" data-domain="${esc(p.macro.id)}" data-risk="${risk}" data-tier="${featuredPriority.get(item.rank_group_key)}" data-lod="${lod}" data-min-scale="${itemMinScale(item)}" title="Landmark score ${Math.round(p.coreScore*100)} · closer to center means broader model coverage and stronger source evidence" style="left:${p.x}px;top:${p.y}px;--node-color:${p.macro.color};--node-scale:${nodeScale}"><span class="coverage">${item.model_count} models</span><b>${esc(mapDisplayName(item))}</b><small>${esc(p.subfield.label)} · core ${Math.round(p.coreScore*100)}</small><span class="best">${esc(formatScore(item.best_score,item.score_unit))}</span></button>`}).join('');
    ...
    function updateSemanticZoom(){
      ...
      $('mapCoverage').textContent=`${visibleNodes.length} / ${formatCount(data.summary.benchmark_group_count)} ${t('benchmarks shown','个 Benchmark 已显示')}`;
      ...
      $('mapScopeLabel').textContent=t(`Top ${perField} per field`,`每个领域前 ${perField} 个`);
    }
    ...
    function updateMapModelMode(){const model=selectedModel();let featuredCoverage=0;document.querySelectorAll('.node').forEach(node=>{const item=catalog.find(entry=>entry.rank_group_key===node.dataset.key);const score=modelScore(item);const covered=!model||Boolean(score);if(model&&score)featuredCoverage+=1;node.classList.toggle('model-muted',!covered);node.classList.toggle('model-covered',Boolean(model&&score));node.querySelector('.best').textContent=model?(score?formatScore(score.s,score.u):'—'):formatScore(item.best_score,item.score_unit);node.querySelector('.coverage').textContent=model?(score?t('model score','模型分数'):t('not reported','未报分')):`${item.model_count} ${t('models','模型')}`;});const catalogCoverage=model?catalog.filter(item=>item.model_scores?.[model.model_id]).length:catalog.length;const domainCoverage=model?new Set(catalog.filter(item=>item.model_scores?.[model.model_id]).map(item=>getMacro(item.domain).id)).size:macroRules.length;$('viewport').classList.toggle('model-mode',Boolean(model));$('mapCoverage').textContent=model?`${featuredCoverage} / ${featured.length} ${t('landmarks','个地图节点')} · ${model.model_name}`:`${featured.length} / ${formatCount(data.summary.benchmark_group_count)} ${t('landmarks shown','个 Benchmark 已显示')}`;$('mapScopeLabel').textContent=model?`${catalogCoverage} ${t('catalog benchmarks','个目录 Benchmark')} · ${domainCoverage} ${t('fields','个领域')}`:t('Top 7 per field','每个领域前 7 个');$('registryScoreHeader').textContent=model?model.model_name:t('Best reported','最佳公开分数');}
    function applyFilters(){const q=state.query.toLowerCase();document.querySelectorAll('.node').forEach(node=>{const item=catalog.find(x=>x.rank_group_key===node.dataset.key);const domainOk=state.domain==='all'||node.dataset.domain===state.domain;const safetyOk=!state.safetyOnly||node.dataset.risk==='true';const queryOk=!q||itemSearchText(item).includes(q);node.classList.toggle('hidden',!domainOk||!safetyOk);node.classList.toggle('dimmed',domainOk&&safetyOk&&!queryOk);});updateMapModelMode();const model=selectedModel();const selectedItem=catalog.find(item=>item.rank_group_key===state.selected);const selectedMacro=selectedItem&&getMacro(selectedItem.domain);const filterMacro=macroRules.find(rule=>rule.id===state.domain);const baseLabel=model?`${model.model_name} ${t('coverage','覆盖')}`:filterMacro?.label||selectedMacro?.label||t('All domains','全部领域');$('statusDomain').textContent=state.safetyOnly?`${baseLabel} · ${t('Safety','安全')}`:baseLabel;if(model&&!state.selected)$('statusSelection').textContent=`${catalog.filter(item=>item.model_scores?.[model.model_id]).length} ${t('reported benchmarks','个已报分 Benchmark')}`;updateSemanticZoom();renderRegistry();if(state.mode==='matrix')renderMatrix();}
    ...
    if(initialItem&&!mapRepresentativeFor(initialItem)&&state.mode==='map')state.mode='registry';
