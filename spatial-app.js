    ...
    function renderMap(){
      ...
      $('viewport').classList.toggle('domain-focus',focused);$('focusBack').hidden=!focused;
      ...
      $('clusterLabels').innerHTML=(focused?`<div class="cluster-label focused-title" style="left:676px;top:34px;color:${focusRule.color}">${esc(focusRule.label)}</div>`:macroRules.map(rule=>`<button class="cluster-label" data-focus-domain="${rule.id}" style="left:${rule.center[0]-220}px;top:${rule.center[1]-218}px;color:${rule.color}" title="${esc(t('Open complete field map','打开完整领域地图'))}">${esc(rule.label)}</button>`).join(''))+subfieldLabels.join('');
