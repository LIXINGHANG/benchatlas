    ...
    function configureMapItems(){const rule=macroRules.find(entry=>entry.id===state.domain);featured=rule?selectDomainFamilies(rule):[...globalFeatured];featuredByFamily=new Map(featured.map(item=>[mapFamilyKey(item),item]));featuredPriority=new Map();featuredLod=new Map();if(rule){const scores=new Map(featured.map(item=>[item.rank_group_key,landmarkCoreScore(item,featured)]));[...featured].sort((a,b)=>scores.get(b.rank_group_key)-scores.get(a.rank_group_key)).forEach((item,index)=>{featuredPriority.set(item.rank_group_key,index<6?'core':'support');featuredLod.set(item.rank_group_key,index<8?'field':index<24?'detail':'deep');});}else macroRules.forEach(entry=>featured.filter(item=>getMacro(item.domain).id===entry.id).forEach((item,index)=>{featuredPriority.set(item.rank_group_key,index<3?'core':'support');featuredLod.set(item.rank_group_key,index<mapTierLimits.field?'field':index<mapTierLimits.detail?'detail':'deep');}));}
    ...
    function positionFocusedNodes(){
      positions.clear();
      const rule=macroRules.find(entry=>entry.id===state.domain);if(!rule)return;
      const center=[900,550];const focusMacro={...rule,center};const groups=new Map();
      featured.forEach(item=>{const subfield=getSubfield(item);if(!groups.has(subfield.id))groups.set(subfield.id,{...subfield,items:[]});groups.get(subfield.id).items.push(item);});
    ...
    function renderMap(){
      ...
      positionNodes();
      buildRelationEdges();
      const focusRule=macroRules.find(rule=>rule.id===state.domain);const focused=Boolean(focusRule);
      ...
      $('clusterLabels').innerHTML=(focused?`<div class="cluster-label focused-title" style="left:676px;top:34px;color:${focusRule.color}">${esc(focusRule.label)}</div><button class="focus-hub-control" id="focusHub" style="--hub-color:${focusRule.color}" aria-label="${esc(t('Reveal more benchmarks','展开更多 Benchmark'))}" title="${esc(t('Click to reveal more benchmarks','点击展开更多 Benchmark'))}"></button>`:macroRules.map(rule=>`<button class="cluster-label" data-focus-domain="${rule.id}" style="left:${rule.center[0]-220}px;top:${rule.center[1]-218}px;color:${rule.color}" title="${esc(t('Open complete field map','打开完整领域地图'))}">${esc(rule.label)}</button>`).join(''))+subfieldLabels.join('');
    ...
    function updateSemanticZoom(){
      ...
      const focusRule=macroRules.find(rule=>rule.id===state.domain);$('mapCoverage').textContent=focusRule?`${visibleNodes.length} / ${featured.length} ${t('benchmark families in this field','个该领域 Benchmark 家族')}`:`${visibleNodes.length} / ${formatCount(data.summary.benchmark_group_count)} ${t('benchmarks shown','个 Benchmark 已显示')}`;
    ...
    function enterDomainFocus(domain,updateUrl=true){state.domain=macroRules.some(rule=>rule.id===domain)?domain:'all';if($('domainSelect'))$('domainSelect').value=state.domain;renderMap();applyFilters();fitView();if(updateUrl)syncUrl();const rule=macroRules.find(entry=>entry.id===state.domain);if(rule)showToast(`${rule.label} · ${featured.length} ${t('benchmark families','个 Benchmark 家族')}`);}
    function clearFilters(){state.modelId='';state.safetyOnly=false;state.query='';$('modelSelect').value='';$('safetyToggle').checked=false;$('search').value='';closeSearch();enterDomainFocus('all',false);syncUrl();}
    function renderFilters(){const models=[...modelCatalog].sort((a,b)=>a.vendor.localeCompare(b.vendor)||a.model_name.localeCompare(b.model_name));$('filters').innerHTML=`<div class="field-control"><label class="toolbar-label" for="domainSelect">Field</label><select id="domainSelect" aria-label="Benchmark field"><option value="all">All benchmark fields</option>${macroRules.map(r=>`<option value="${r.id}">${r.label}</option>`).join('')}</select></div><div class="field-control model-control"><label class="toolbar-label" for="modelSelect">Model</label><select id="modelSelect" aria-label="Filter by model"><option value="">All models</option>${models.map(model=>`<option value="${esc(model.model_id)}">${esc(model.vendor)} · ${esc(model.model_name)}</option>`).join('')}</select></div><label class="safety-toggle"><input id="safetyToggle" type="checkbox"><span>Safety layer</span></label><button class="filter-reset" id="clearFilters" aria-label="Reset filters" title="Reset filters">↺</button>`;$('domainSelect').value=macroRules.some(rule=>rule.id===state.domain)?state.domain:'all';state.domain=$('domainSelect').value;$('modelSelect').value=modelById.has(state.modelId)?state.modelId:'';state.modelId=$('modelSelect').value;$('safetyToggle').checked=state.safetyOnly;$('domainSelect').addEventListener('change',event=>enterDomainFocus(event.target.value));$('modelSelect').addEventListener('change',event=>{state.modelId=event.target.value;const active=catalog.find(item=>item.rank_group_key===state.selected);const score=active&&modelScore(active);applyFilters();if(active&&score)selectBenchmark(active.rank_group_key,score.g,state.modelId);else if(active)closeInspector(false);syncUrl();if(state.modelId){const model=selectedModel();showToast(`${model.model_name} · ${catalog.filter(item=>item.model_scores?.[state.modelId]).length} reported benchmarks`);}});$('safetyToggle').addEventListener('change',event=>{state.safetyOnly=event.target.checked;applyFilters();syncUrl();});$('clearFilters').addEventListener('click',clearFilters);}
    ...
    function openSearchResult(key){const item=catalog.find(entry=>entry.rank_group_key===key);if(!item)return;state.safetyOnly=false;$('safetyToggle').checked=false;state.query='';$('search').value='';closeSearch();enterDomainFocus(getMacro(item.domain).id,false);const representative=mapRepresentativeFor(item);switchMode('map');selectBenchmark(key);setTimeout(()=>centerOn((representative||item).rank_group_key),0);syncUrl();}
    ...
    if(initialItem&&!mapRepresentativeFor(initialItem)&&state.mode==='map')enterDomainFocus(getMacro(initialItem.domain).id,false);
