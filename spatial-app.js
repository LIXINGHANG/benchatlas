    ...
    const state = {scale:.72,x:40,y:18,drag:false,startX:0,startY:0,baseX:0,baseY:0,domain:initialHash.get('domain')||'all',modelId:modelById.has(initialHash.get('model'))?initialHash.get('model'):'',safetyOnly:initialHash.get('safety')==='1',query:'',selected:null,mode:initialMode,activeVariants:[],activeItem:null,matrixModelIds:new Set((initialHash.get('models')||'').split(',').filter(Boolean)),initialProtocol:initialHash.get('protocol')||''};
    const $ = id => document.getElementById(id);
    const esc = value => String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
    const formatScore = (score,unit) => score == null || score === '' ? '—' : `${score}${unit==='%'?'%':''}`;
    function syncUrl(){const params=new URLSearchParams();if(state.selected)params.set('benchmark',state.selected);const protocol=$('comparisonSelect')?.value;if(state.selected&&protocol)params.set('protocol',protocol);if(state.mode!=='map')params.set('view',state.mode);if(state.domain!=='all')params.set('domain',state.domain);if(state.modelId)params.set('model',state.modelId);if(state.safetyOnly)params.set('safety','1');if(state.matrixModelIds.size)params.set('models',[...state.matrixModelIds].join(','));const hashValue=params.toString();history.replaceState(null,'',`${location.pathname}${location.search}${hashValue?`#${hashValue}`:''}`);}
    function prepareShareLink(){syncUrl();showToast('Share URL is ready in the address bar');}
    async function applyUrlState(){const params=new URLSearchParams(location.hash.replace(/^#/,''));const requestedMode=params.get('view');state.mode=['map','registry','matrix','ranking'].includes(requestedMode)?requestedMode:'map';const requestedDomain=params.get('domain');state.domain=macroRules.some(rule=>rule.id===requestedDomain)?requestedDomain:'all';state.modelId=modelById.has(params.get('model'))?params.get('model'):'';state.safetyOnly=params.get('safety')==='1';state.matrixModelIds=new Set((params.get('models')||'').split(',').filter(Boolean));if($('domainSelect'))$('domainSelect').value=state.domain;if($('modelSelect'))$('modelSelect').value=state.modelId;if($('safetyToggle'))$('safetyToggle').checked=state.safetyOnly;const benchmark=params.get('benchmark');const hasBenchmark=catalog.some(item=>item.rank_group_key===benchmark);if(!hasBenchmark)state.selected=null;applyFilters();if(hasBenchmark&&!positions.has(benchmark)&&state.mode==='map')state.mode='registry';switchMode(state.mode);if(hasBenchmark&&state.mode!=='ranking')await selectBenchmark(benchmark,params.get('protocol')||catalog.find(item=>item.rank_group_key===benchmark)?.model_scores?.[state.modelId]?.g||'',state.modelId);else if(!hasBenchmark)closeInspector(false);}
    ...
    async function selectBenchmark(key,preferredProtocolId,preferredModelId){
      ...
      const renderComparison=(groupId,preferredModelId)=>{
        ...
        const focusedModelId=preferredModelId||state.modelId;let groups=comparison.modelGroups.slice(0,8);const focusedGroup=focusedModelId&&comparison.modelGroups.find(group=>group.model_id===focusedModelId);if(focusedGroup&&!groups.some(group=>group.model_id===focusedModelId))groups=[...groups.slice(0,7),focusedGroup];
        $('ranking').innerHTML=groups.map((group,index)=>{const allModelRows=page.rows.filter(row=>(isRankingEligible(row)?(row.base_model_id||row.model_id):(row.reported_model_id||row.model_id))===group.model_id);const setupCount=new Set(allModelRows.map(row=>row.comparability_group_id)).size;const configuration=group.best.model_configuration||'';const descriptor=group.ranking_eligible?(configuration?`Best config: ${configuration}`:'Standard configuration'):`${String(group.entity_type||'reference').replaceAll('_',' ')} · reference only`;return `<div class="rank-row" data-group="${index}"><span class="num">${group.ranking_eligible?String(index+1).padStart(2,'0'):'REF'}</span><span><b>${esc(group.model_name)}</b><small>${esc(group.vendor)} · ${esc(descriptor)}${setupCount>1?` · ${setupCount} setups`:group.rows.length>1?` · ${group.rows.length} source rows`:''}</small></span><span class="score">${esc(formatScore(group.best.score,group.best.score_unit))}</span></div>`;}).join('')||'<p class="method">No reported rows.</p>';
        const showGroup=index=>{
          const group=groups[index];if(!group)return;document.querySelectorAll('.rank-row').forEach(row=>row.classList.toggle('active',Number(row.dataset.group)===index));state.activeVariants=group.rows;$('variantPicker').style.display=group.rows.length?'block':'none';$('variantSelect').innerHTML=group.rows.map((row,rowIndex)=>{const configuration=row.model_configuration||'Standard';return `<option value="${rowIndex}">${esc(formatScore(row.score,row.score_unit))} · ${esc(configuration)} · ${esc(sourceLabel(row))}${isFirstPartyRow(row)?' · official model vendor':group.rows.length>1?` · source ${rowIndex+1}`:''}</option>`;}).join('');$('methodStatus').textContent=group.rows.length>1?`${group.rows.length} configuration/source rows`:'1 source row';updateEvidence(group.rows[0],item);
          const alternatives=page.rows.filter(row=>(isRankingEligible(row)?(row.base_model_id||row.model_id):(row.reported_model_id||row.model_id))===group.model_id&&row.comparability_group_id!==comparison.id);
          $('relatedReports').innerHTML=alternatives.slice(0,5).map(row=>`<button class="related-report" data-comparison="${esc(row.comparability_group_id)}" data-model="${esc(group.model_id)}"><b>Also reported · ${esc(sourceLabel(row))}${isFirstPartyRow(row)?' · official model vendor':''}</b><span>${esc(formatScore(row.score,row.score_unit))}</span><small>${esc(row.comparability_group_label||'Source-scoped reported setup')}</small></button>`).join('');
        ...
        const initialIndex=Math.max(0,groups.findIndex(group=>group.model_id===focusedModelId));showGroup(initialIndex);document.querySelectorAll('.rank-row').forEach((row,index)=>row.addEventListener('click',()=>showGroup(index)));
      };
      const focusedComparison=comparisonGroups.find(group=>group.modelGroups.some(model=>model.model_id===(preferredModelId||state.modelId)));const initialComparisonId=comparisonGroups.some(group=>group.id===preferredProtocolId)?preferredProtocolId:focusedComparison?.id||comparisonGroups[0]?.id;$('comparisonSelect').onchange=event=>{renderComparison(event.target.value,preferredModelId||state.modelId);syncUrl();};renderComparison(initialComparisonId,preferredModelId||state.modelId);$('benchmarkLink').href=`/benchmarks/${slugify(item.rank_group_key)}/`;syncUrl();showToast(`${displayName(item)} · ${groupedRows.length} unique models`);
    }

    function updateEvidence(row,item){if(!row)return;$('methodNote').innerHTML=structuredMethodHtml(row);const comparisonBadge=row.comparability_status==='strict'?'shared protocol':'source scoped';const entityBadge=String(row.entity_type||'model').replaceAll('_',' ');const badgeList=[comparisonBadge,entityBadge,...(isSafety(item)?['safety layer']:[]),...(row.protocol_badges||[])].filter(Boolean);$('badges').innerHTML=[...new Set(badgeList)].map((b,i)=>`<span class="badge ${i===0||b==='safety layer'?'red':''}">${esc(b.replaceAll('_',' '))}</span>`).join('');const configuration=row.model_configuration?` · configuration: ${row.model_configuration}`:'';$('sourceSummary').textContent=`${row.ranking_eligible===false?'Reference entity':'Base model'}: ${row.base_model_name||row.model_name||'unknown'}${configuration} · reported in: ${sourceLabel(row)} · ${comparisonBadge}`;$('sourceLocation').textContent=row.evidence_location||'source located';$('evidenceQuote').textContent=row.evidence_quote||'No short evidence quote available.';$('sourceLink').href=String(row.source_url||'#').split(/;\s*/)[0];}

    function updateMapModelMode(){const model=selectedModel();let featuredCoverage=0;document.querySelectorAll('.node').forEach(node=>{const item=catalog.find(entry=>entry.rank_group_key===node.dataset.key);const score=modelScore(item);const covered=!model||Boolean(score);if(model&&score)featuredCoverage+=1;node.classList.toggle('model-muted',!covered);node.classList.toggle('model-covered',Boolean(model&&score));node.querySelector('.best').textContent=model?(score?formatScore(score.s,score.u):'—'):formatScore(item.best_score,item.score_unit);node.querySelector('.coverage').textContent=model?(score?'model score':'not reported'):`${item.model_count} models`;});const catalogCoverage=model?catalog.filter(item=>item.model_scores?.[model.model_id]).length:catalog.length;const domainCoverage=model?new Set(catalog.filter(item=>item.model_scores?.[model.model_id]).map(item=>getMacro(item.domain).id)).size:macroRules.length;$('viewport').classList.toggle('model-mode',Boolean(model));$('mapCoverage').textContent=model?`${featuredCoverage} of ${featured.length} landmarks · ${model.model_name}`:`${featured.length} of ${formatCount(data.summary.benchmark_group_count)} landmarks shown`;$('mapScopeLabel').textContent=model?`${catalogCoverage} catalog benchmarks · ${domainCoverage} fields`:'Top 7 per field';$('registryScoreHeader').textContent=model?model.model_name:'Best reported';}
    function applyFilters(){const q=state.query.toLowerCase();document.querySelectorAll('.node').forEach(node=>{const item=catalog.find(x=>x.rank_group_key===node.dataset.key);const domainOk=state.domain==='all'||node.dataset.domain===state.domain;const safetyOk=!state.safetyOnly||node.dataset.risk==='true';const queryOk=!q||itemSearchText(item).includes(q);node.classList.toggle('hidden',!domainOk||!safetyOk);node.classList.toggle('dimmed',domainOk&&safetyOk&&!queryOk);});updateMapModelMode();const model=selectedModel();const selectedItem=catalog.find(item=>item.rank_group_key===state.selected);const selectedMacro=selectedItem&&getMacro(selectedItem.domain);const filterMacro=macroRules.find(rule=>rule.id===state.domain);const baseLabel=model?`${model.model_name} coverage`:filterMacro?.label||selectedMacro?.label||'All domains';$('statusDomain').textContent=state.safetyOnly?`${baseLabel} · Safety`:baseLabel;if(model&&!state.selected)$('statusSelection').textContent=`${catalog.filter(item=>item.model_scores?.[model.model_id]).length} reported benchmarks`;renderRegistry();if(state.mode==='matrix')renderMatrix();}

    function clearFilters(){state.domain='all';state.modelId='';state.safetyOnly=false;state.query='';$('domainSelect').value='all';$('modelSelect').value='';$('safetyToggle').checked=false;$('search').value='';closeSearch();applyFilters();syncUrl();}
    function renderFilters(){const models=[...modelCatalog].sort((a,b)=>a.vendor.localeCompare(b.vendor)||a.model_name.localeCompare(b.model_name));$('filters').innerHTML=`<div class="field-control"><label class="toolbar-label" for="domainSelect">Field</label><select id="domainSelect" aria-label="Benchmark field"><option value="all">All benchmark fields</option>${macroRules.map(r=>`<option value="${r.id}">${r.label}</option>`).join('')}</select></div><div class="field-control model-control"><label class="toolbar-label" for="modelSelect">Model</label><select id="modelSelect" aria-label="Filter by model"><option value="">All models</option>${models.map(model=>`<option value="${esc(model.model_id)}">${esc(model.vendor)} · ${esc(model.model_name)}</option>`).join('')}</select></div><label class="safety-toggle"><input id="safetyToggle" type="checkbox"><span>Safety layer</span></label><button class="filter-reset" id="clearFilters" aria-label="Reset filters" title="Reset filters">↺</button>`;$('domainSelect').value=macroRules.some(rule=>rule.id===state.domain)?state.domain:'all';state.domain=$('domainSelect').value;$('modelSelect').value=modelById.has(state.modelId)?state.modelId:'';state.modelId=$('modelSelect').value;$('safetyToggle').checked=state.safetyOnly;$('domainSelect').addEventListener('change',event=>{state.domain=event.target.value;applyFilters();syncUrl();});$('modelSelect').addEventListener('change',event=>{state.modelId=event.target.value;const active=catalog.find(item=>item.rank_group_key===state.selected);const score=active&&modelScore(active);applyFilters();if(active&&score)selectBenchmark(active.rank_group_key,score.g,state.modelId);else if(active)closeInspector(false);syncUrl();if(state.modelId){const model=selectedModel();showToast(`${model.model_name} · ${catalog.filter(item=>item.model_scores?.[state.modelId]).length} reported benchmarks`);}});$('safetyToggle').addEventListener('change',event=>{state.safetyOnly=event.target.checked;applyFilters();syncUrl();});$('clearFilters').addEventListener('click',clearFilters);}
    function filteredCatalog(){const q=state.query.toLowerCase();return catalog.filter(item=>(state.domain==='all'||getMacro(item.domain).id===state.domain)&&(!state.modelId||Boolean(item.model_scores?.[state.modelId]))&&(!state.safetyOnly||isSafety(item))&&(!q||itemSearchText(item).includes(q)));}
    ...
    function renderRegistry(){$('registryBody').innerHTML=filteredCatalog().slice(0,100).map(item=>{const macro=getMacro(item.domain);const score=modelScore(item);const model=selectedModel();const config=score?.n&&score.n!=='Standard'?` · ${score.n}`:'';return `<tr data-key="${esc(item.rank_group_key)}"><td><b>${esc(displayName(item))}</b><small>${esc(item.metric_name)} · ${esc(item.score_unit)}${item.benchmark_variant?` · ${esc(item.benchmark_variant)}`:''}</small></td><td>${esc(macro.label)}<small>${esc(item.domain.replaceAll('_',' '))}${isSafety(item)?' · safety':''}</small></td><td>${item.model_count}<small>base models</small></td><td>${item.vendor_count}</td><td class="score">${esc(score?formatScore(score.s,score.u):formatScore(item.best_score,item.score_unit))}<small>${esc(model?.model_name||item.best_model)}${esc(config)}</small></td><td>${protocolChips(item)}</td></tr>`}).join('');document.querySelectorAll('#registryBody tr').forEach(row=>row.addEventListener('click',()=>{const item=catalog.find(entry=>entry.rank_group_key===row.dataset.key);const score=modelScore(item);selectBenchmark(row.dataset.key,score?.g||'',state.modelId);}));}
    ...
    async function renderOverallRanking(){
      $('overallRankingBody').className='ranking-loading';$('overallRankingBody').textContent='Loading overall ranking…';
      try{const overall=await loadRankingData();const rankings=overall.rankings||[];$('rankingModelCount').textContent=rankings.length;$('rankingGroupCount').textContent=overall.benchmarkGroupCount||'—';
        $('overallRankingBody').className='';$('overallRankingBody').innerHTML=`<table class="overall-ranking"><thead><tr><th>Rank</th><th>Base model</th><th>RPI ceiling</th><th>Coverage</th><th>Confidence</th></tr></thead><tbody>${rankings.map(row=>{const model=modelCatalog.find(item=>item.model_id===row.model_id);const path=model?`/models/${encodeURIComponent(model.model_id)}/`:'#';const width=Math.max(0,Math.min(100,Number(row.index_score)||0));return `<tr data-path="${esc(path)}"><td class="overall-rank">${String(row.overall_rank).padStart(2,'0')}</td><td class="model-cell"><b>${esc(row.model_name)}</b><small>${esc(row.vendor)} · ${row.configuration_count||1} reported configuration${row.configuration_count===1?'':'s'}</small></td><td class="rpi-cell"><div class="rpi-value"><span>${esc(row.index_score)}</span><small>RPI</small></div><div class="rpi-bar"><i style="width:${width}%"></i></div></td><td class="coverage-cell">${row.benchmark_count} benchmarks<small>${row.domain_count} reported domains · raw ${row.raw_score}</small></td><td class="confidence-cell"><span class="confidence-pill">${esc(row.confidence)}</span><small>${row.report_count} source reports</small></td></tr>`;}).join('')}</tbody></table>`;
      ...
      }catch(error){$('overallRankingBody').className='ranking-loading';$('overallRankingBody').textContent='Overall ranking could not be loaded. Refresh to retry.';rankingPromise=null;}
    }
    ...
    switchMode(state.mode);if(initialBenchmark){const initialItem=catalog.find(item=>item.rank_group_key===initialBenchmark);const score=modelScore(initialItem);selectBenchmark(initialBenchmark,state.initialProtocol||score?.g||'',state.modelId);}else closeInspector(false);
