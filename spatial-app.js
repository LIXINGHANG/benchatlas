    ...
    const safetyTaxonomy=data.taxonomy?.safety_alignment||{};
    const safetyViewRule={id:safetyTaxonomy.view_id||'safety_alignment',label:t(safetyTaxonomy.label_en||'Safety & Alignment',safetyTaxonomy.label_zh||'安全与对齐'),color:safetyTaxonomy.color||'#b42318',center:[900,540],crossDomain:true};
    const domainViewRules=[...macroRules,safetyViewRule];
    const safetyCategoryById=new Map((safetyTaxonomy.categories||[]).map(category=>[category.id,category]));
    ...
    const getSafetyCategory=item=>{const category=safetyCategoryById.get(item?.safety_category);return{id:item?.safety_category||'uncategorized',label:t(item?.safety_category_label_en||category?.label_en||'Uncategorized safety',item?.safety_category_label_zh||category?.label_zh||'未分类安全评测')}};
    const matchesDomainView=(item,viewId)=>viewId==='all'||(viewId===safetyViewRule.id?isSafety(item):getMacro(item).id===viewId);
    const getViewSubfield=(item,viewId)=>viewId===safetyViewRule.id?getSafetyCategory(item):getSubfield(item);
    ...
    function updateSemanticZoom(){
      ...
      const focusRule=domainViewRules.find(rule=>rule.id===state.domain);$('mapCoverage').textContent=focusRule?`${visibleNodes.length} / ${featured.length} ${focusRule.crossDomain?t('safety benchmark families','个安全与对齐 Benchmark 家族'):t('benchmark families in this field','个该领域 Benchmark 家族')}`:`${visibleNodes.length} / ${formatCount(data.summary.benchmark_group_count)} ${t('benchmarks shown','个 Benchmark 已显示')}`;
    ...
    async function selectBenchmark(key,preferredProtocolId,preferredModelId){
      ...
      const related=highlightRelations(mapKey);const groupedRows=groupRowsByModel(page.rows);const comparisonGroups=comparisonGroupsForRows(page.rows);const macro=getMacro(item);const activeView=state.domain===safetyViewRule.id?safetyViewRule:macro;const subfield=getViewSubfield(item,activeView.id);
    ...
    function enterDomainFocus(domain,updateUrl=true){state.domain=domainViewRules.some(rule=>rule.id===domain)?domain:'all';if($('domainSelect'))$('domainSelect').value=state.domain;renderMap();applyFilters();fitView();if(updateUrl)syncUrl();const rule=domainViewRules.find(entry=>entry.id===state.domain);if(rule)showToast(`${rule.label} · ${featured.length} ${rule.crossDomain?t('safety benchmark families','个安全与对齐 Benchmark 家族'):t('benchmark families','个 Benchmark 家族')}`);}
    ...
    function renderRegistry(){$('registryBody').innerHTML=filteredCatalog().slice(0,100).map(item=>{const macro=getMacro(item);const activeView=state.domain===safetyViewRule.id?safetyViewRule:macro;const subfield=getViewSubfield(item,activeView.id);const score=modelScore(item);const model=selectedModel();const config=score?.n&&score.n!=='Standard'?` · ${score.n}`:'';return `<tr data-key="${esc(item.rank_group_key)}"><td><b>${esc(displayName(item))}</b><small>${esc(item.metric_name)} · ${esc(item.score_unit)}${item.benchmark_variant?` · ${esc(item.benchmark_variant)}`:''}</small></td><td>${esc(activeView.label)}<small>${esc(subfield.label)}${isSafety(item)&&activeView.id!==safetyViewRule.id?' · safety':''}</small></td><td>${item.model_count}<small>base models</small></td><td>${item.vendor_count}</td><td class="score">${esc(score?formatScore(score.s,score.u):formatScore(item.best_score,item.score_unit))}<small>${esc(model?.model_name||item.best_model)}${esc(config)}</small></td><td>${protocolChips(item)}</td></tr>`}).join('');document.querySelectorAll('#registryBody tr').forEach(row=>row.addEventListener('click',()=>{const item=catalog.find(entry=>entry.rank_group_key===row.dataset.key);const score=modelScore(item);selectBenchmark(row.dataset.key,score?.g||'',state.modelId);}));}
