...
- 通过语义缩放逐步展示更多 Benchmark：默认显示 21 个核心地标，领域层最多 49 个，详情层 63 个，深度层 70 个。
...
- 查看“**公开榜单平均百分位**”，汇总全部已报分基础模型在公开可比组中的归一化名次。
...
整体排名汇总全部已报分基础模型在**公开可比组中的平均归一化名次**，不代表默认产品体验、API 延迟、调用成本或绝对智能水平。

1. 每个至少包含一个基础模型公开报分的可比组都会参与计算；单模型组没有相对名次，因此记为中性百分位 50。
2. 在组内将模型名次转换成 0–100 的百分位分数。
3. 同一 Benchmark family 中的多次出现先取平均，避免别名、指标、变体和重复报告各自获得一份完整权重。
4. 直接平均模型全部已观察到的 Benchmark family 百分位；能力领域不再改变分数权重。
5. 不设置 family、领域、组内模型数或厂商数门槛。family 覆盖较少的模型向 50 收缩；family 数、领域覆盖和独立报告数共同决定置信度。
6. Agent 系统、Checkpoint、Baseline 等非基础模型实体不会参与排名。
7. 当同一可比组中出现同一基础模型的多个配置时，采用该组内排名最高的合格公开配置。

由于数据来自厂商公开报告，整体排名仍可能受到 Benchmark 选择、选择性报分和来源构成偏差影响。分数更高只表示它在当前已观察榜单中的平均位置更高，并不能证明模型在所有任务上都更强。做出重要判断前应检查底层报分记录和置信度。
...
- 方向表示数学、软件工程、终端系统、工具、长上下文、医疗或 AI 研发与自我改进等二级领域；
- 距离中心的位置由模型覆盖（60%）、来源覆盖（25%）和方法说明信号（15%）共同决定；
- 语义缩放默认显示每个领域 3 个核心地标，再依次扩展到 7、9、10 个 Benchmark 家族；
