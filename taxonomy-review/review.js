(() => {
  ...
  const colorByDomain = {reason:'#28568c',code:'#087f78',agent:'#ae6a13',multi:'#69518e',language:'#4e7d37',expert:'#914d38'};
  ...
  const benchmarks = [...familyMap].map(([name, items]) => {
    ...
    return {
      ...
      original: {primary_domain:primaryDomain, subfield},
      ...
      isSafety: items.some(item => item.is_safety),
  ...
  function classification(benchmark) {
    return state.assignments[benchmark.name] || benchmark.original;
  }
  ...
  function moveBenchmark(name, primaryDomain, subfield) {
    ...
    snapshot();
    if (benchmark.original.primary_domain === primaryDomain && benchmark.original.subfield === subfield) delete state.assignments[name];
    else state.assignments[name] = {primary_domain:primaryDomain, subfield};
  ...
  function cardHtml(benchmark) {
    ...
    return `<article class="card ${className} ${state.selectedName===benchmark.name?'selected':''}" draggable="true" data-name="${esc(benchmark.name)}" style="--domain-color:${colorByDomain[classification(benchmark).primary_domain]}"><h3>${esc(benchmark.name)}</h3><div class="card-meta"><span>${benchmark.items.length} group${benchmark.items.length===1?'':'s'}</span><span>${benchmark.modelCount} models</span><strong>${Math.round(benchmark.confidence*100)}%</strong><span>${esc(sourceLabels[benchmark.taxonomySource]||benchmark.taxonomySource)}</span>${benchmark.isSafety?'<span>SAFETY</span>':''}</div></article>`;
  }
  ...
  function renderInspector() {
    ...
    $('inspector').innerHTML = `<article><p class="eyebrow">${esc(sourceLabels[benchmark.taxonomySource]||benchmark.taxonomySource)} · confidence ${Math.round(benchmark.confidence*100)}%</p><h2>${esc(benchmark.name)}</h2><p class="variant-count">${benchmark.items.length} rank groups · ${benchmark.modelCount} models · ${benchmark.reportCount} reports${benchmark.isSafety?' · SAFETY':''}</p><div class="original"><div><span>原一级领域</span><b>${esc(domainById.get(benchmark.original.primary_domain)?.label_zh)}</b></div><div><span>原二级领域</span><b>${esc(subfieldLabel(benchmark.original.primary_domain,benchmark.original.subfield))}</b></div></div><div class="field"><label for="inspectorDomain">一级领域</label><select id="inspectorDomain">${domainOptions}</select></div><div class="field"><label for="inspectorSubfield">二级领域</label><select id="inspectorSubfield">${subfieldOptions}</select></div><button class="apply" id="applyClassification">应用分类</button><div class="field"><label>审核状态</label><div class="review-buttons">${Object.entries(statusLabels).map(([status,label])=>`<button data-status="${status}" class="${review.status===status?'active':''}">${label}</button>`).join('')}</div></div><div class="field"><label for="reviewNote">审核备注</label><textarea id="reviewNote" placeholder="分类依据、需要复核的问题或修改理由">${esc(review.note||'')}</textarea></div>${benchmark.reason?`<div class="field"><label>现有分类理由</label><div>${esc(benchmark.reason)}</div></div>`:''}<div class="variant-list"><h3>关联分组</h3>${variants}</div></article>`;
  ...
  function buildPatch() {
    ...
    for (const benchmark of changed) {
      ...
      benchmarkOverrides[benchmark.name] = {
        ...existing,
        source_domain: domain.source_domains[0],
        subfield: current.subfield,
        confidence: 1,
        reason: reviewFor(benchmark.name).note || `Manual taxonomy review: ${benchmark.original.primary_domain}/${benchmark.original.subfield} -> ${current.primary_domain}/${current.subfield}`
      };
    }
    ...
    reader.onload = () => {
      try {
        ...
        for (const record of patch.review_records || []) {
          ...
          if (record.changed && validClassification(record.current?.primary_domain,record.current?.subfield)) state.assignments[benchmark.name] = record.current;
